/* =========================================================================
 *  Source for C Language Professional Edition
 * =========================================================================
 *
 *	All Rights Reserved, Copyright(c) Lawrence 2023
 *
 * ---------------------------------------------------------------------- */
#include "mongoose.h"
...
//curl localhost:8000/api/verify -d '[123.38, -2.72]'

static void fn(struct mg_connection *c, int ev, void *ev_data, void *fn_data) {

  if (ev == MG_EV_HTTP_MSG) {
    printf("New HTTP Message\n");

    struct mg_http_message *hm = (struct mg_http_message *) ev_data;
    if (mg_http_match_uri(hm, "/api/identify")) {
        printf("New Identify Message\n");
      // Expecting JSON array in the HTTP body, e.g. [ 123.38, -2.72 ]
      double num1, num2;
      if (mg_json_get_num(hm->body, "$[0]", &num1) && // Extract first number
          mg_json_get_num(hm->body, "$[1]", &num2)) { // Extract second number
        // Success! create JSON response
        int ret = identify(); //add parameters such as scanner ID
        if(ret == 1) {
            mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                      "{%m:%g}\n",
                      mg_print_esc, 0, "identify error result", num1 + num2);
        } else if(ret == 0) {
            mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                      "{%m:%g}\n",
                      mg_print_esc, 0, "identify result", num1 + num2);
        }else {
            mg_http_reply(c, 500, NULL, "Error when issuing identify command\n");
        } 
      } else { //cannot extract
        mg_http_reply(c, 500, NULL, "Unsupported identify request format JSON array expected\n");
      }
    }  else if (mg_http_match_uri(hm, "/api/verify")) {
        printf("New Verify Message\n");
      // Expecting JSON array in the HTTP body, e.g. [ 123.38, -2.72 ]
      double num1, num2;
      if (mg_json_get_num(hm->body, "$[0]", &num1) && // Extract first number
          mg_json_get_num(hm->body, "$[1]", &num2)) { // Extract second number
        // Success! create JSON response
        int ret = verify(); //add parameters such as scanner ID
        if(ret == 1) { //failure
            mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                      "{%m:%g}\n",
                      mg_print_esc, 0, "verify error result", num1 + num2);
        } else if(ret == 0) { //success
            mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                      "{%m:%g}\n",
                      mg_print_esc, 0, "verify result", num1 + num2);
        }else {
            mg_http_reply(c, 500, NULL, "Error when issuing verify command\n");
        } 
      } else { //cannot extract
        mg_http_reply(c, 500, NULL, "Unsupported verify request format JSON array expected\n");
      }
    } 
    else if (mg_http_match_uri(hm, "/api/status")) {              // On /api/hello requests,
      mg_http_reply(c, 200, "", "{%m:%d}\n",
                    MG_ESC("status"), 1);                   // Send dynamic JSON response
    } else if(mg_http_match_uri(hm, "/api/home")){                                                // For all other URIs,
...
int main(int argc, char **argv) {  
  ...
  printf("Started a new HTTP listener at localhost port 8002\n");
...
/* -------------------------------------------------------------------------
	*identify. ...will use scanner ID from request
	...
	BioAPI_sample_C_Identify stores enrollment data and capture data on
	the memory.
 ...
 *	verify
	...
	BioAPI_sample_C_Verify stores enrollment data “BIRData.dat”,
	capture data “BIRCapData.dat”, and silhouette image data
